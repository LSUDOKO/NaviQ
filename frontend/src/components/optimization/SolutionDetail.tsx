      ...
      >
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(solution.weights)
              ...
              .map(([key, weight]) => (
                <span
                  key={key}
                  className="chip bg-ink-800 border border-ink-bright text-txt-secondary"
                  title={`Weight ${num(weight * 100, 0)}%`}
                >
                  {OBJECTIVE_LABELS[key] ?? key}
                  <span className="metric text-txt-tertiary">{num(weight * 100, 0)}%</span>
                </span>
              ))}
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-ink-700/70 grid gap-3 sm:grid-cols-4 text-2xs">
          {[
            ...
            <div key={stat.label}>
              <span className="text-txt-quiet">{stat.label} </span>
              <span className="metric text-txt-secondary">{stat.value}</span>
            </div>
          ))}
        </div>
      </Panel>
            ...
            <tbody>
              {solution.voyages.map((v, index) => (
                <tr
                  ...
                  className={`border-b border-ink-800/70 last:border-0 cursor-pointer transition-colors ${
                    index === activeVoyage ? "bg-ink-850" : "hover:bg-ink-850/40"
                  }`}
                >
                  <td className="px-4 py-3 text-txt-primary">{v.vessel_name}</td>
                  <td className="px-4 py-3 text-txt-secondary text-xs">{v.route_name}</td>
                  <td className="px-4 py-3 text-txt-secondary text-xs">
                    {FUEL_SHORT[v.fuel_id] ?? v.fuel_id}
                  </td>
                  <td className="px-4 py-3 text-right metric text-xs text-txt-secondary hidden sm:table-cell">
                    {num(v.voyage_totals.average_speed_kn, 1)} kn
                  </td>
                  <td className="px-4 py-3 text-right metric text-xs text-txt-secondary hidden md:table-cell">
                  ...
                  <td className="px-4 py-3 text-center hidden lg:table-cell">
                    {v.shore_power.selected ? (
                      ...
                      <span className="text-2xs text-txt-quiet">—</span>
                    )}
                  </td>
      ...
      {voyage?.shore_power.detail && (
        ...
        >
          <div className="grid gap-5 sm:grid-cols-3">
            <div>
              <p className="text-2xs text-txt-tertiary mb-1">Running auxiliaries</p>
              <p className="metric text-lg text-txt-primary">
                {tonnes(voyage.shore_power.detail.auxiliary_option.ghg_wtw_t, 2)}
              </p>
              <p className="text-2xs text-txt-quiet">
                {usd(voyage.shore_power.detail.auxiliary_option.cost_usd)}
              </p>
            </div>
            <div>
              <p className="text-2xs text-txt-tertiary mb-1">Drawing from the grid</p>
              <p className="metric text-lg text-txt-primary">
            ...
            <div>
              <p className="text-2xs text-txt-tertiary mb-1">Decision</p>
              <p
                className={`metric text-lg ${
                  voyage.shore_power.selected ? "text-cii-a" : "text-txt-secondary"
                }`}
              >
