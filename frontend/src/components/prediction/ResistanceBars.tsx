  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-4">
        <span className="text-xs text-txt-tertiary">Total resistance</span>
        <span className="metric text-xl text-txt-primary">
            ...
            <div key={component.key}>
              <div className="flex items-baseline gap-2.5">
                <span
                  ...
                  aria-hidden="true"
                />
                <dt className="text-xs text-txt-secondary flex-1">{component.label}</dt>
                <dd className="metric text-xs text-txt-secondary">
                  {num(value, 1)} <span className="unit">kN</span>
                </dd>
                <dd className="metric text-xs text-txt-tertiary w-12 text-right">{pct(share, 0)}</dd>
              </div>
              <p className="text-2xs text-txt-quiet ml-4.5 mt-0.5 leading-relaxed pl-1">
                {component.note}
              </p>
            </div>
          );
